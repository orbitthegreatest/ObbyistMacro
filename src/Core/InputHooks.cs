...
public static class InputHooks
{
    ...
    /// <summary>
    /// While true, every mouse event (moves, buttons, wheel) is swallowed by the
    /// hook, so the focused game never sees mouse input. Used during the FPS
    /// sequence so the menu hover can't be disturbed. SetCursorPos itself is not
    /// affected — only delivery of the events it generates is suppressed.
    /// </summary>
    public static volatile bool BlockMouse;
    ...
    private static IntPtr MouseCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        ...
        {
            long msg = wParam.ToInt64();
            if (BlockMouse && msg >= WM_MOUSEFIRST && msg <= WM_MOUSELAST)
                return new IntPtr(1);   // swallow the event: the game never sees it
